                    ...
                    for xcFolderVersion in xcFolderVersions:
                        for catFolderVersion in catFolderVersions:
                            ...
                            if xcFolVerLower == catFolVerLower and len(xcFolderVersions) >= 1 and len(catFolderVersions) >= 1:
                                # ? match exactly one character - "src" (old version) & "src" (new version) can be applied
                                # the lengh of "src" path is only 1 so there is no need to conduct a for-loop
                                ...
                                if xcFolPathLower == catFolPathLower:
                                    ...
                                    if cnt > 0:
                                        ...
                                        with open(os.path.join(catFolderPath[0], 'path.txt'), 'w') as note:
                                            note.write('- The src file(s) is/are copied from "{}"\n- Latest Update: {} {}'.format(xcFolderPath[0], now, local_tz))

def autoAllFiles(year: int, srcPapiBusinessUnit: str, srcXcPath: str, dstCatPath: str):
    srcPapiPaths = glob.glob(os.path.join('T:\RSG Chicago\CAT Modeling\PAPI Reports\*', srcPapiBusinessUnit.strip(), '*\Report'))
...
# inputs
year = 2022
srcPapiBusinessUnit = "RT Tampa and Jupiter - Hume"
...
autoAllFiles(year, srcPapiBusinessUnit, srcXcPath, dstCatPath)
