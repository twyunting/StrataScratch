####################################################################################################
## Name: {Account Modeling File Automations}
...
# define functions
def genFolderNames(year: int, sourcePapiPath: str, destinationCatPath: str) -> list[str]:
    # list to store file names
    names = []
    versions = []
    year = "_" + str(year)
    # save folder names to arrays
    for path in os.listdir(sourcePapiPath):
        # check if current path is a folder
        if os.path.isdir(os.path.join(sourcePapiPath, path)) and len(re.findall(year, os.path.join(sourcePapiPath, path))) == 1:
            # remove spaces at the beginning and at the end of the string
            versions.append(path.strip())
            names.append(path.split("_", 1)[0].strip())
    # generate folders
    for name in names:
        if not os.path.exists(os.path.join(destinationCatPath, name)):
            dir = os.path.join(destinationCatPath, name)
            os.mkdir(dir)
...
def genFolders(res: dict[str, str], destinationCatPath: str):
    ...
    for key, value in res.items():
        ...
        if digit != -1 and len(digitList) > 0 and len(re.findall("_R[0-9]+", key)) == 1 and not os.path.exists(os.path.join(destinationCatPath, value, "rev_" + str(digit))):
            Rs_folder = os.path.join(destinationCatPath, value, "rev_" + str(digit))
            os.mkdir(Rs_folder)
            if os.path.exists(Rs_folder):
                for i in range(len(subFolders)):
                    subFolder = os.path.join(Rs_folder, subFolders[i])
                    os.mkdir(subFolder)
        elif key.count("_") == 1 and not os.path.exists(os.path.join(destinationCatPath, value, "ori")):
            ori_folder = os.path.join(destinationCatPath, value, "ori")
            os.mkdir(ori_folder)
            if os.path.exists(ori_folder):
                for i in range(len(subFolders)):
                    subFolder = os.path.join(ori_folder, subFolders[i])
                    os.mkdir(subFolder)
        elif key.lower().count("_op") == 1 and len(re.findall("_R[0-9]+", key)) == 0 and not os.path.exists(os.path.join(destinationCatPath, value, "ori")):
            ori_folder = os.path.join(destinationCatPath, value, "ori")
            os.mkdir(ori_folder)
            if os.path.exists(ori_folder):
                for i in range(len(subFolders)):
...
def copyFiles(res: dict[str, str], sourcePapiPath: str, sourceXcPath: str, destinationCatPath: str):
    for key, value in res.items():
        # final
        for file in glob.glob(os.path.join(sourcePapiPath, key, '*.xlsx')):
            # find a file name
            excelFileName = str(os.path.split(file)[1])
            # cur_file should be a list: len(list) >= 1
            curFolder = glob.glob(os.path.join(destinationCatPath, value, '*/final'))
            for i in curFolder:
                ...
                if not os.path.exists(copiedFileName) and len(re.findall("ori", accVersion)) == 1 and len(re.findall("R[0-9]+", excelFileName)) < 1:
                    ...
                    shutil.copy(file, copiedFileName)
        # working
        for data in glob.glob(os.path.join(sourcePapiPath, key, '*.txt')):
            textDataName = str(os.path.split(data)[1])
            # cur_file should be a list: len(list) >= 1
            curFolder = glob.glob(os.path.join(destinationCatPath, value, '*/working'))
            for j in curFolder:
                # find an account version: ori, rev_12345
                ...
                if not os.path.exists(copiedFileName) and len(re.findall("ori", accVersion)) == 1 and len(re.findall("R[0-9]+", textDataName)) < 1 and textDataName != "PAPILog.txt" and textDataName != "RMSLog.txt":
                    ...
                    shutil.copy(data, copiedFileName)
        # source
        for xc in os.listdir(sourceXcPath):
            if os.path.isdir(os.path.join(sourceXcPath, xc.strip())):
                xcPaths = os.path.join(sourceXcPath, xc.strip())
                if str(os.path.split(xcPaths)[1].lower()) == value.lower():
                    xcFolderVersions = glob.glob(os.path.join(xcPaths, '*'))
                    catFolderVersions = glob.glob(os.path.join(destinationCatPath, value, '*'))
...
def autoAllFiles(year: int, sourcePapiBU: str, sourceXcPath: str, destinationCatPath: str):
    sourcePapiPaths = glob.glob(os.path.join('T:\RSG Chicago\CAT Modeling\PAPI Reports\*', sourcePapiBU.strip(), '*\Report'))
    # iterate four paths: CAT/Compact Edition, CAT/Full Edition, Xceedance/Compact Edition, Xceedance/Full Edition
    for sourcePapiPath in sourcePapiPaths:
        versions, names = genFolderNames(year, sourcePapiPath, destinationCatPath)
        res = oldNewNamesDict(versions, names)
        genFolders(res, destinationCatPath)
        copyFiles(res, sourcePapiPath, sourceXcPath, destinationCatPath)
        print("Completed:", sourcePapiPath)


# inputs
year = 2022
sourcePapiBU = "RT Tampa and Jupiter - Hume"
sourceXcPath = r'T:\Xceedance\RT\RT Tampa (and Jupiter) - Hume\2022'
destinationCatPath = r'U:\WORKS\Acc_Tools\PJAC06 - Files_Automation\JEM_test_files\JEM2022\destination1'

autoAllFiles(year, sourcePapiBU, sourceXcPath, destinationCatPath)
