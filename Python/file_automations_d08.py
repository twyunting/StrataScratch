...
# define functions
def genFolderNames(year: int, source_path: str, destination_path: str) -> list[str]:
    ...
    for path in os.listdir(source_path):
        ...
        if os.path.isdir(os.path.join(source_path, path)) and len(re.findall(year, os.path.join(source_path, path))) == 1:
...
def genFolders(res: dict[str, str], destination_path: str) -> None:
...
def copyFiles(res: dict[str, str], source_path: str, sourceXC_path: str, destination_path: str) -> None:
    for key, value in res.items():
        # final
...
def autoAllFiles(year, source_BU, sourceXC_path, destination_path):
    ...
    for source_path in source_PAPIs:
        versions, names = genFolderNames(year, source_path, destination_path)
        res = oldNewNamesDict(versions, names)
        genFolders(res, destination_path)
        copyFiles(res, source_path, sourceXC_path, destination_path)
        print("Completed:", source_path)
...
# inputs
year = 2022
source_BU = "RT Tampa and Jupiter - Hume"
sourceXC_path = r'T:\Xceedance\RT\RT Tampa (and Jupiter) - Hume\2022'
#source_path = r'U:\WORKS\Acc_Tools\PJAC06 - Files_Automation\JEM_test_files\JEM2022\source'
destination_path = r'U:\WORKS\Acc_Tools\PJAC06 - Files_Automation\JEM_test_files\JEM2022\destination1'

autoAllFiles(year, source_BU, sourceXC_path, destination_path)

"""
versions, names = genFolderNames(source_path, destination_path, year)
res = oldNewNamesDict(versions)
genFolders(res)
copyFiles(res, sourceXC_path)
"""
